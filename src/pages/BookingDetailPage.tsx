...
import { Clock, MapPin, Download, XCircle, Armchair, CreditCard } from 'lucide-react';
    ...
    <div style={{ paddingBottom: 80 }}>
      ...
      <div style={{ padding: '20px 16px 0', display: 'flex', flexDirection: 'column', gap: 8 }}>
        {booking.status === 'confirmed' && (
          <button className="w-full bg-accent text-white text-[14px] font-semibold flex items-center justify-center gap-2 active:opacity-80 transition-opacity" style={{ height: 48, borderRadius: 8 }}>
            <Download size={16} /> {t('booking.downloadTicket')}
          </button>
        )}
        {booking.can_cancel && (
          <button className="w-full border border-danger text-danger text-[14px] font-semibold flex items-center justify-center gap-2 active:opacity-80 transition-opacity" style={{ height: 48, borderRadius: 8 }}>
            <XCircle size={16} /> {t('booking.cancel')}
          </button>
        )}
      </div>
    </div>
