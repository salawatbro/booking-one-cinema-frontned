...
export function SchedulePage() {
  ...
  return (
    <div style={{ paddingBottom: 80 }}>
                ...
                <div className="flex flex-col gap-1.5" style={{ marginTop: 6 }}>
                  {movieShowtimes.map((st) => (
                    <button
                      ...
                      className="flex items-center justify-between border border-border bg-bg-card active:opacity-70 transition-opacity"
      ...
      {movies.length === 0 && (
        <p className="text-center text-[13px] text-text-tertiary" style={{ paddingTop: 60 }}>
          Bu kunda seans topilmadi
        </p>
      )}
    </div>
  );
